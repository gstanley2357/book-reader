...
env:
  ...
  clear:
    ...
    # Use book_reader-db for a db accessory server on same machine via local kamal docker network.
    # DB_HOST: 192.168.0.2
...
volumes:
  - "book_reader_storage:/rails/storage"
...
# docker build -t book_reader .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name book_reader book_reader
